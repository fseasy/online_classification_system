...
def save_svm_model(model , f_path , gzip_on=False) :
    if type(f_path) == file :
        of = f_path 
    else :
        try :
            if gzip_on :
                of = gzip.open(f_path , 'wb')
            else :
                of = open(f_path , 'w')
        except IOError , e :
            print >> sys.stderr , "failed to save svm model : open file error "
            return
    pickle.dump(model , of)
    if type(f_path) != file :
        of.close()

def load_svm_model(f_path , gzip_on=False) :
    if type(f_path) == file :
        input_f = f_path 
    else :
        try :
            ...
            print >> sys.stderr , "failed to load svm model : open file error "
            return None
...
def build_test_doc_ids(pos_t , neg_t) :
    doc_ids = []
